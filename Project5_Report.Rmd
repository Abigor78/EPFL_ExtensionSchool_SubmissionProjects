...
qry_01_highest_grossing_dramas_2010 <- GET(
...
qry_01_highest_grossing_dramas_2010 <- qry_01_highest_grossing_dramas_2010 %>% 
  content() %>% 
  flatten()

# Extract the original_title as a 1 column tibble
qry_list_original_title1 <- qry_01_highest_grossing_dramas_2010 %>% 
...
qry_list_title1 <- qry_01_highest_grossing_dramas_2010 %>% 
  map(extract, "title") %>% 
  flatten() %>% 
  tibble()  %>%
  rename("title"=".") %>% 
...
qry_list_release_date1 <- qry_01_highest_grossing_dramas_2010 %>% 
...
result_01_highest_grossing_dramas_2010 <- bind_cols(
...
result_01_highest_grossing_dramas_2010 %>%  
  head(10) %>% 
  rename(
...
qry_02_Will_Ferrell_and_Liam_Neeson  <- GET(
  str_glue("https://api.themoviedb.org/3/",
  "discover/movie?api_key=cae3b93dd5e2d16994763563ea0a2eb5",
...
result_02_Will_Ferrell_and_Liam_Neeson %>%  
  rename(
    "Original title"=original_title,
    "Title"=title,
    "Release date"=release_date
...
qry_03_Tom_Cruise_for_kids  <- GET(
  str_glue("https://api.themoviedb.org/3/",
  "discover/movie?api_key=cae3b93dd5e2d16994763563ea0a2eb5",
...
# Flatten the result to extract the data
qry_Pixar_company <- qry_Pixar_company %>% 
  content() %>% 
  flatten()
